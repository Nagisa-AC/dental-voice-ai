...
from app.core.database import supabase, safe_supabase_insert
from app.intelligence.dental_faq_matcher import (
    DentalFAQMatcher,
    create_practice_config_from_db_record,
    identify_practice_from_vapi_data
)
from app.core.schemas import IntentType, WebhookResponse, IntentAnalysisRequest, IntentAnalysisResponse
import logging
import json
import time
...
async def vapi_webhook(request: Request) -> WebhookResponse:
    ...
    try:
        ...
        # Analyze intent
        intent_result = faq_matcher.analyze_transcript(
        ...
        response = WebhookResponse(
            results=[
                {
                    "toolCallId": call_data.get("tool_call_id", "unknown"),
