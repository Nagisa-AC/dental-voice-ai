...
from db.models.database_models import AuditAction, AuditResource
...
class AuditMiddleware(BaseHTTPMiddleware):
    ...
    async def _log_audit_event(
        ...
        try:
            # Skip audit logging for webhook endpoints to avoid errors
            if description and "/webhooks/incoming_call" in description:
                return
                
            # Simple audit logging without database for now
            logger.info(f"Audit: {action.value} {resource_type.value} - {description} - Success: {success}")
        except Exception as e:
