  ...
  const execute = useCallback(async (...args: any[]): Promise<T | null> => {
    ...
    if (useCache && cacheKey) {
      ...
      if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
        setState(prev => ({ ...prev, data: cached.data as T | null, loading: false, error: null }));
...
export function usePost<T = unknown, D = unknown>(
  ...
  const apiCall = useCallback((data: D) => axios.post<T>(url, data) as Promise<AxiosResponse<T>>, [url]);
  return useApi(apiCall, options);
}
...
export function usePut<T = unknown, D = unknown>(
  ...
  const apiCall = useCallback((data: D) => axios.put<T>(url, data) as Promise<AxiosResponse<T>>, [url]);
  return useApi(apiCall, options);
}
